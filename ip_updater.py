...
HOSTS_FILE = CONFIGURATION_DIRECTORY + '/hosts.data'
        ...
        try:
            ...
            for host in hosts:
                if ('always_update' not in host
                        ...
                        or 'username' not in host
                        or 'password' not in host):
                ...
                if host['always_update'] or host_ip != public_ip:
                    update_host_ip(host['host_name'], host['updater'], host['username'], host['password'], public_ip)
                else:
...
def get_host_ip(host_name):
    try:
        ...
        error_message = f'Cannot resolve {host_name}'
...
def update_host_ip(host_name, updater, username, password, public_ip):
    updater = updater.replace('<HOST_NAME>', host_name).replace('<NEW_IP>', public_ip)

    try:
        response = requests.get(updater, auth=(username, password))
    except requests.exceptions.ConnectionError:
    ...
    if response_code == 'good':
        info_message = f'Host {host_name} new IP: {public_ip}'
        write_to_syslog(syslog.LOG_INFO, info_message)
        return
    elif response_code == 'nochg':
        info_message = f'Host {host_name} IP is already up to date'
        write_to_syslog(syslog.LOG_INFO, info_message, disable_telegram_notification=True)
        return
    else:
        error_message = f'Unrecognized updater response:{os.linesep * 2}{response}'
        write_to_syslog(syslog.LOG_ERR, error_message)
        return
